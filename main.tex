...
\begin{frame}{Configuración física}
\vspace*{2em}
\begin{figure}[h!]
\centering
...
\note{\large \vfill
	\begin{center}
		\begin{enumerate}
			\item Explicar de fuera a dentro PC->vBox->Ubuntu->{Ryu,Mininet}
			\vspace{2em}	
			%\item 
			%\vspace{2em}
			\vfill
...
\note{\large \vfill
	\begin{center}
		\begin{enumerate}
			\item Como el mostrado anteriormente pero más detallado 
			\vspace{2em}
			\item igual para los dos casos de uso, solo cambia que en uno se usa Miniedit.
			\vspace{2em}	
            \item SUPONEMOS QUE YA TENEMOS CONFIGURADO/INSTALADO TODO LO DE LA FIGURA.
			\vspace{2em}	
			\vfill
		\end{enumerate}
\end{center}}
...
\vspace*{-1em}
\begin{exampleblock}{\centering Definición del escenario de aplicación}
\end{exampleblock}
%\begin{figure}[h!]
%	\centering
...
\note{\large \vfill
	\begin{center}
		\begin{enumerate}
			...
			\vspace{2em}
            \item  Ping H1-H2 para ver que funciona.
			\vspace{2em}
			\vfill
		\end{enumerate}
\end{center}}
...
\note{\large \vfill
	\begin{center}
		\begin{enumerate}
			\item  Se vería solo hasta se ha llamado. para los swtiches. Despues hacemos un ping para que se tenga constancia de los hosts.
			\vspace{2em}	
			\item ir a la diapo anterior y que las rutas concuerdan.
			\vspace{2em}
            \item  Costes (se ha puesto que el ancho de banda del enlace sea el mismo que el de referencia, así el coste de enlace es 1, y coste total=nº enlaces).
			\vspace{2em}
...
\note{\large \vfill
	\begin{center}
		\begin{enumerate}
        \item  Veamos si las rutas encontradas han sido instaladas en s5. Este es el primer paso. haremos flows->grupos-> buckets->actions
			\vspace{2em}
	        \item Cada flow se encarga del matching de pkts IP y ARP (uno de cada)
            \vspace{2em}
            \item Se ridirigen los dos al mismo group:...
			\vspace{2em}
            \item \textcolor{red}{YO QUITABA ESTA DIAPO Y PONIA DIRECTAMENTE LA SIGUIENTE, LA DE LOS GROUPS (SON DEMASIADAS DIAPOS SI NO).}
            \vspace{2em}	
			\vfill
		\end{enumerate}
\end{center}}
...
\note{\large \vfill
	\begin{center}
		\begin{enumerate}
			\item  Hay un solo grupo con 2 buckets en la Group Table
			\vspace{2em}	
			\item bucket weight se corresponde con el coste: 2/3=4/6=> 2/(2+3)=4/(4+6)=0.4
			\vspace{2em}
			\item  
			\vspace{2em}
			\vfill
...
\note{\large \vfill
	\begin{center}
		\begin{enumerate}
			...
			\vspace{2em}	 
			\item  en Open vSwitch ultimas versiones para trafico TCP se realiza un hash simetrico (igual ambos sentidos) con los puertos de origen y destino y se elige el bucket teniendo en cuenta el hash (hash1->rutaX siempre...) 
			\vspace{2em}
...
\note{\large \vfill
	\begin{center}
		\begin{description}
			...
			\vspace{2em}
			\item Caso de uso incorpora QoS en una red SDN para ver que facil se implementa.. Se pretende
enseñar cómo, mediante QoS, se pueden transferir los datos de una red en función
de la prioridad basada en los tipos de datos, y reservar ancho de banda para una
comunicación con el objetivo de comunicar dos puntos con un constante ancho de
banda en la red.
			\vspace{2em}
			\vfill
		\end{description}
